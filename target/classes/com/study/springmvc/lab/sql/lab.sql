...
/*相同的symbol並加總*/
select s.symbol, sum(s.share) as share from fundstock s group by s.symbol
/*fund向左關聯fundstock*/
select f.fid, f.fname, f.createtime,
s.sid as fundstock_sid, s.fid as fundstock_fid,
s.symbol as fundstock_symbol, s.share as fundstock_share 
from fund f left join fundstock s on s.fid=f.fid 
order by f.fid limit 5 offset 0